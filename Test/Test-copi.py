...
if ultima_vela is not None and len(ultima_vela) > 0:
    ...
    print(f"🕒 Hora actual del servidor MT5: {hora_servidor.strftime('%Y-%m-%d %H:%M:%S')}")
else:
...
for fecha in sorted(df['fecha_mt5'].unique()):
    ...
    for i in range(1, len(df_dia) - 1):
        ...
        riesgo_monetario = capital_inicial * risk_pct_loss
        valor_pip = 10
        riesgo_pips = abs(entrada - sl) * 10000
        lote_utilizado = round(riesgo_monetario / (riesgo_pips * valor_pip), 2) if riesgo_pips != 0 else 0
...
plt.xlabel("Fecha")
plt.ylabel("Capital ($)")
plt.xticks(fechas, [f.strftime("%Y-%m-%d") for f in fechas], rotation=45)
plt.grid(True)
plt.tight_layout()
